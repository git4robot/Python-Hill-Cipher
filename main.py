...
def crack(plaintext, ciphertext, alpha):
  ...
  plainM = sp.Matrix([[alpha.index(plaintext[0]), alpha.index(plaintext[2])], [alpha.index(plaintext[1]), alpha.index(plaintext[3])]])
  cipherM = sp.Matrix([[alpha.index(ciphertext[0]), alpha.index(ciphertext[2])], [alpha.index(ciphertext[1]), alpha.index(ciphertext[3])]])
  ...
  for pw in passwords:
    ...
    for common in commonPasswords:
      ...
      try:
        ...
        if encrypt(common, matrix, alpha) == pw[0:len(common)+(len(common)%2)]:
          return matrix, usernames[passwords.index(pw)], decrypt(pw, matrix, alpha)
      except ValueError:
        pass
...
print("hill 2x2 encode:", encrypt(plaintext, sp.Matrix([[9, 14], [22, 7]]), alpha29))
print("hill 2x2 decode:", decrypt(encrypt(plaintext, sp.Matrix([[9, 14], [22, 7]]), alpha29), sp.Matrix([[9, 14], [22, 7]]), alpha29))
print("hill 2x2 crack:", crack(plaintext, encrypt(plaintext, sp.Matrix([[9, 14], [22, 7]]), alpha29), alpha29))
